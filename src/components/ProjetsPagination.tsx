...
type ProjetsPaginationProps = {
  projetsData: PROJETSData[];
};

const ProjetsPagination: React.FC<ProjetsPaginationProps> = ({ projetsData }) => {
  ...
  const currentProjets = projetsData.slice(indexOfFirstProjet, indexOfLastProjet);

  // Calculer le nombre total de pages
  const totalPages = Math.ceil(projetsData.length / projetsPerPage);
...
Démo : {projet.url && <Link target="_blank" href={projet.url} style={{color:"blue", marginBottom:10}}>{projet.url}</Link>}
</div>
                
                <div style={{ marginTop: 15 }}>
                <a
  href={`https://api.whatsapp.com/send?phone=2250565110441&text=${encodeURIComponent(
    `Bonjour, je suis intéressé par ce projet : ${he.encode(projet.title)}.
Prix : ${parseFloat(projet.prix).toLocaleString('fr-FR', { style: 'currency', currency: 'XOF' })}
Image : ${projet.image}`
  )}`}
  target="_blank"
...
export default ProjetsPagination;
