// components/RchretiennesPagination.tsx
"use client"
import React, { useState } from "react";
import { RCHRETIENNESData } from "../types"; // ou ../src/types si le fichier est dans un sous-dossier
import { FaWhatsapp } from "react-icons/fa";
import he from "he";

type RchretiennesPaginationProps = {
  RCHRETIENNESData: RCHRETIENNESData[];
};

const RchretiennesPagination: React.FC<RchretiennesPaginationProps> = ({ RCHRETIENNESData }) => {
  const [currentPage, setCurrentPage] = useState(1);
  const rchretiennesPerPage = 6;

  // Calculer l'index des rchretiennes à afficher en fonction de la page courante
  const indexOfLastRchretiennes = currentPage * rchretiennesPerPage;
  const indexOfFirstRchretiennes = indexOfLastRchretiennes - rchretiennesPerPage;
  const currentRchretienness = RCHRETIENNESData.slice(indexOfFirstRchretiennes, indexOfLastRchretiennes);

  // Calculer le nombre total de pages
  const totalPages = Math.ceil(RCHRETIENNESData.length / rchretiennesPerPage);
  ...
  return (
    <div>
      ...
      {currentRchretienness.map((rchretienne, index) => (                 
              <div key={index} className="col-md-4 mb-4">
                ...
                >
                  {he.decode(rchretienne.title)}
                </div>
                <br/>
                <img
                  src={rchretienne.image}
                  alt="rchretienne en vente"
                ...
                >
                    Prix : {parseFloat(rchretienne.prix).toLocaleString('fr-FR', { style: 'currency', currency: 'XOF' })}
                </div>
                <div
  ...
  dangerouslySetInnerHTML={{
    __html: rchretienne.description,
  }}
/>
                <div style={{ marginTop: "10px" }}>
                <a
  href={`https://api.whatsapp.com/send?phone=2250565110441&text=${encodeURIComponent(
    `Bonjour, je suis intéressé par cet article : ${he.decode(rchretienne.title)}.
    Prix : ${parseFloat(rchretienne.prix).toLocaleString('fr-FR', { style: 'currency', currency: 'XOF' })}
    Image : ${rchretienne.image}`
  )}`}
  target="_blank"
...
export default RchretiennesPagination;
